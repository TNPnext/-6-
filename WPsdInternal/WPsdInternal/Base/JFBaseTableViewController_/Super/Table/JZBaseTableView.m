...
@interface JZBaseTableView () //<SRRefreshDelegate>

@property (nonatomic, copy, readwrite) NSString *offset;

@end

@implementation JZBaseTableView
{
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        if (self.enablePullDownToRefresh) {
            ...
            _refreshController.tintColor = RGB(100, 106, 125);
...
- (void)finishDataFetchWithModels:(NSArray *)models hasMore:(BOOL)hasMore currentOffset:(NSString *)currentOffset
{
    ...
    if (!models || [models count] == 0) {
        if (self.models.count == 0) {
            self.alert = _enableNilModelsAlertTest;
        }
    ...
    self.alert = nil; // < implemented extension viewController category
                                    ...
                                    cuurentCount:(NSUInteger)currentCount
{
    ...
    _upRefreshButton = [[UIButton alloc]initWithFrame:CGRectMake(0.0, 0.0, CGRectGetWidth(self.frame), 40)];
    _upRefreshButton.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
    _upRefreshButton.backgroundColor = [UIColor whiteColor];
...
@implementation JZBaseTableView (internet)
