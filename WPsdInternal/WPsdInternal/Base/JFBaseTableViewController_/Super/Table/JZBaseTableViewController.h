 ...
 *     上拉刷新的类型
 */
typedef NS_ENUM (NSUInteger, JZUpPushToRefreshType)
{
    JZUpPushToRefreshTypeNormal = 0,  //  表格滑动到最后一条数据时，自动开始加载下一批表格数据。
    JZUpPushToRefreshTypeTapped = 1,  //  表格滑动到最后出现点击加载更多按钮，点击加载下一批表格数据。
};
 ...
 *     控制器刷新的类型
 */
typedef NS_ENUM (NSUInteger, JZRefreshType)
{
    JZRefreshTypeNormal   = 0,  //  初次加载数据时刷新
    JZRefreshTypeDownPush = 1,  //  下啦刷新
    JZRefreshTypeUpPush   = 2,  //  上拉刷新
};
 ...
 */
@interface JZBaseTableViewController : JZModelTableViewController
{
...
/**
 *     刷新控件颜色， default is RGB(255, 133, 97).
 */
@property (nonatomic, strong) UIColor *refreshControllerTintColor;
...
@interface JZBaseTableViewController (internet)
