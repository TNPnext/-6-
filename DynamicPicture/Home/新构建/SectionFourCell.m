...
@implementation SectionFourCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        NSArray *imaA = @[@"11",@"22",@"21",@"21"];
        
        NSArray *titA = @[@"时时彩",@"山东11选5",@"安徽快3",@"快3"];
...
{
    if (self.delegate &&[self.delegate respondsToSelector:@selector(SectionFourCellCellClick:)])
    {
        [self.delegate SectionFourCellCellClick:sender];
    }
}
