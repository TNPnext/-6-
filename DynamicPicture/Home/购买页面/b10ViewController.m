...
#import "b10ViewController.h"

@interface b10ViewController ()<UIScrollViewDelegate>
{
    NSArray *_titArray;
    NSArray *_tipArray;
    UIView *_topView;
...
@implementation b10ViewController
...
-(void)createtMainView
{
    ...
    _bgV.delegate = self;
    _bgV.bounces = NO;
    _bgV.contentSize = CGSizeMake(kMainWidth, 75*7+150);
    ...
    _sectionV = [[UIView alloc]initWithFrame:CGRectMake(0, 30, kMainWidth, 75*7+150)];
    ...
    switch (sender.tag) {
        case 0:
        {
            ...
            gzView.btnBlock = ^(NSInteger index) {
                ...
                {
                    //玩法规则
                    url = @"http://client.leaicai.com//news/10024_1.htm?requestType=1&version=1.0.3&appVersion=1";
                    title = @"玩法规则";
                }
...
-(void)section1
{
    ...
    NSArray *sectiontA = @[@" 第一位",@" 第二位",@" 第三位",@" 第四位",@" 第五位"];
    for (int s= 0; s<5; s++)
    {
