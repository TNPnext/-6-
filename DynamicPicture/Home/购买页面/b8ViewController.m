...
#import "b8ViewController.h"

@interface b8ViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation b8ViewController
    ...
    [MNetworkManager getBuyDataWithUrl:self.dataUrl Success:^(id data)
     {
         ...
         _dataArray = data[@"dataList"];
         NSDictionary *dict = data[@"firstIssue"];
         _sellTime = dict[@"sellEndTime"];
...
{
    b2Cell *cell = [tableView dequeueReusableCellWithIdentifier:@"cellID"];
    if (!cell)
    {
        cell = [[b2Cell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cellID"];
    }
    cell.indexPath = indexPath;
    NSDictionary *dic = _dataArray[indexPath.section];
    NSArray *arr = dic[@"matches"];
    NSArray *data = [b1Model mj_objectArrayWithKeyValuesArray:arr];
    b1Model *model = data[indexPath.row];
    model.sellEndTime = _sellTime;
    cell.model = model;
...
-(void)buttonClick:(UIButton *)sender
{
    ...
    b2Cell *cell = (b2Cell *)sender.superview.superview.superview;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    _title.text = [NSString stringWithFormat:@"%@(%@) %@场比赛",dic[@"dayKey"],dic[@"dayOfWeekStr"],(dic[@"matchCount"]==nil)?dic[@"totalMatch"]:dic[@"matchCount"]];
